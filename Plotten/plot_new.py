...
# Arrange nodes for each connected component separately
pos = {}
x_offset = 0
y_offset = 0
for idx, subgraph in enumerate(subgraphs):
    try:
        ...
        pos.update(sub_pos)
        x_offset += len(hierarchy) * 3  # Increase horizontal spacing
        ...
        sub_pos = {node: (x + x_offset, y + y_offset - idx * 5) for node, (x, y) in sub_pos.items()}
        pos.update(sub_pos)
        x_offset += len(sub_pos) * 3
    y_offset -= 10  # Increase vertical spacing between components
